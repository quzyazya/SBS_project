...
% if request.query_params.get('error') == 'disable_2fa_first':
    <div class="alert alert-danger alert-dismissible fade show fixed-bottom mx-auto" style="max-width: 500px; left: 0; right: 0;">⚠️ Сначала отключите 2FA в настройках!<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>
% endif
% if request.query_params.get('error') == 'invalid_code':
    <div class="alert alert-danger alert-dismissible fade show fixed-bottom mx-auto" style="max-width: 500px; left: 0; right: 0;">⚠️ Неверный код подтверждения!<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>
% endif
% if request.query_params.get('error') == 'sms_failed':
    <div class="alert alert-danger alert-dismissible fade show fixed-bottom mx-auto" style="max-width: 500px; left: 0; right: 0;">⚠️ Не удалось отправить SMS. Попробуйте позже.<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>
% endif